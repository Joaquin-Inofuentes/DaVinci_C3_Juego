...
public class AccionesJugador : MonoBehaviour
{
    ...
    public void Disparar()
    {
        ...
        if (arma == null || arma.balasEnCartucho <= 0)
        {
    ...
    public float tiempoRecarga = 1.5f; // esto se toma del arma
    ...
    public void IniciarRecarga()
    {
        ...
        if (arma == null || arma.municionTotal <= 0 || arma.balasEnCartucho == arma.capacidadCartucho)
            return;

        AudioSource.PlayClipAtPoint(arma.sonidoRecarga, transform.position);
        timerRecarga = arma.tiempoRecarga;
        estaRecargando = true;
    }
    ...
    public void AtaqueMelee()
    {
        ...
        {
            ZombieEnemigo enemigo = hit.collider.GetComponent<ZombieEnemigo>();
            if (enemigo != null)
            {
