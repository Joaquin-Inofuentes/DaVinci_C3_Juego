...
public class ZombieEnemigo : MonoBehaviour
{
    ...
    {
        Transform objetivo = other.transform;
        if (!other.gameObject.GetComponent<Jugador>()) return;
    ...
    void Atacar()
    {
        ...
        if (objetivoActual.GetComponent<Jugador>().salud == 0)
        {
            return;
        }
        ...
        if (timerAtaqueEspecial <= 0)
        {
            objetivoActual.GetComponent<Jugador>()?.RecibirDanio(dañoEspecial);
        ...
        else if (timerAtaque <= 0)
        {
            ...
            {
                objetivoActual.GetComponent<Jugador>()?.RecibirDanio(dañoNormal);
            }
